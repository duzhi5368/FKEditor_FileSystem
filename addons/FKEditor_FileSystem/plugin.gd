...
#-------------------------------------------------
const PLUGIN_DIR = "res://addons/FKEditor_FileSystem/"
var View = preload("res://addons/FKEditor_FileSystem/Scripts/View.gd")
#-------------------------------------------------
var fsview = preload("./UI/FileSystemView.tscn").instance()
var config_dialog = preload("./UI/ViewEditor.tscn").instance()
#-------------------------------------------------
...
func load_views():
	var file = File.new()
	if file.open(PLUGIN_DIR + "Config/config.json", File.READ) != OK:
		if file.open(PLUGIN_DIR + "Config/defaultConfig.json", File.READ) == OK:
...
func save_views():
	...
	var file = File.new()
	file.open(PLUGIN_DIR + "Config/config.json", File.WRITE)
